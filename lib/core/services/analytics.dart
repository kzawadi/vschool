...
class OptOutAwareFirebaseAnalytics implements FirebaseAnalytics {
  ...
  final String _ANALYTICS_ENABLED_KEY = "analyticsEnabled";

  OptOutAwareFirebaseAnalytics(
    this._wrapped,
  );
  ...
  Future<Null> logViewItem(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.logViewItem(
          itemId: itemId,
          itemName: itemName,
  ...
  Future<Null> logSpendVirtualCurrency(
      ...
      @required num value}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logSpendVirtualCurrency(
          itemName: itemName,
  ...
  Future<Null> logSelectContent(
      {@required String contentType, @required String itemId}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logSelectContent(
  ...
  Future<Null> logSearch(
      ...
      String travelClass}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logSearch(
          searchTerm: searchTerm,
          numberOfNights: numberOfNights,
          numberOfRooms: numberOfRooms,
          numberOfPassengers: numberOfPassengers,
  ...
  Future<Null> logPurchaseRefund(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.logPurchaseRefund(
          currency: currency, value: value, transactionId: transactionId);
    }
  ...
  Future<Null> logPresentOffer(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.logPresentOffer(
          itemId: itemId,
          itemName: itemName,
          itemCategory: itemCategory,
          quantity: quantity,
  ...
  Future<Null> logPostScore(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.logPostScore(
  ...
  Future<Null> logEcommercePurchase(
      ...
      String travelClass}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logEcommercePurchase(
          currency: currency,
          value: value,
  ...
  Future<Null> logEarnVirtualCurrency(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.logEarnVirtualCurrency(
  ...
  Future<Null> logCampaignDetails(
      ...
      String cp1}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logCampaignDetails(
          source: source,
          medium: medium,
          campaign: campaign,
          term: term,
  ...
  Future<Null> logBeginCheckout(
      ...
      String travelClass}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logBeginCheckout(
          value: value,
          currency: currency,
          transactionId: travelClass,
          numberOfNights: numberOfNights,
          numberOfRooms: numberOfRooms,
          numberOfPassengers: numberOfPassengers,
          origin: origin,
  ...
  Future<Null> logAddToWishlist(
      ...
      String itemLocationId}) async {
    if (await _shouldTrackUser()) {
      return await _wrapped.logAddToWishlist(
          itemId: itemId,
          itemName: itemName,
          itemCategory: itemCategory,
          quantity: quantity,
  ...
  Future<Null> logAddToCart(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.logAddToCart(
          itemId: itemCategory,
          itemName: itemName,
          itemCategory: itemCategory,
          quantity: quantity,
  ...
  Future<Null> setCurrentScreen(
    ...
    if (await _shouldTrackUser()) {
      return await _wrapped.setCurrentScreen(
...
// ----------------------------------->
  ...
  Future<void> logLevelEnd({String levelName, int success}) {
    // TODO: implement logLevelEnd
    throw UnimplementedError();
  }

  @override
  Future<void> logLevelStart({String levelName}) {
    // TODO: implement logLevelStart
    throw UnimplementedError();
  }
