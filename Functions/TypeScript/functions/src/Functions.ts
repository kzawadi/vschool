...
export async function studentParentAutoEntry(eventSnapshot: any, context: any) {
    ...
    let inJsonFormat = Object.assign(newValue, { id: newValue!.id });
    ...
    if (connections != null) {
        ...
        connectionsArray.map(async (value, index) => {
            let connectionProfileRef = await getProfileRef(schoolCode, country, UserType.PARENT, value);
            const key = (index + 1) + '';
    ...
    let ref2 = db.collection('Schools').doc(country).collection(schoolCode).doc('Students').collection(standard + division).doc(studentId);
...
export async function messageIdAutoEntry(eventSnapshot: any, context: any) {
    ...
    let pathToChat = `/Schools/${country}/${schoolCode}/Chats/${standard}/Chat/${chatId}/`;
    ...
    let ref1 = db.collection(`/Schools/${country}/${schoolCode}/Chats/${standard}/Parent-Teacher/${from}/`).doc(to);
    let ref2 = db.collection(`/Schools/${country}/${schoolCode}/Chats/${standard}/Parent-Teacher/${to}/`).doc(from);
